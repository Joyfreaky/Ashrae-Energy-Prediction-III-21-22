...
train_df = pd.read_feather(root_data/'train.feather')
...
df_meta['groupNum_train'] = df_meta['site_id'].astype(
    'int')*10 + df_meta['meter'].astype('int')
df_meta
...
sample_submission = sample_submission.merge(
    df_meta, on=['building_id', 'meter'], how='left')
sample_submission['meter_reading_log1p'] = np.log1p(
    sample_submission['meter_reading'])
sample_submission

# %% [code]

df_allData = pd.concat([train_df.merge(df_meta, on=['building_id', 'meter'], how='left')[['building_id', 'meter', 'groupNum_train', 'timestamp', 'meter_reading_log1p']],
...
df_median = df_median.pivot_table(
    values='median_building_meter', columns='groupNum_train', index='timestamp')
